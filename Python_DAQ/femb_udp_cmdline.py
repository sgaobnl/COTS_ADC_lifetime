...
class FEMB_UDP:
    ...
    def get_rawdata_packets(self, val, jumbo_flag = True):
        numVal = int(val)
        ...
        while ( lost_pkg_fg == True ):
            ...
            sock_data = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # Internet, UDP
            sock_data.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock_data.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 8192*(val+1000))
            sock_data.bind(('',self.UDP_PORT_HSDATA))
            ...
            if (pkg_chk):
                ...
                if (lost_pkg_fg == True):
                    if  (defe_pkg_cnt <10):
                        ...
                        print "defective packages found!!!"
                        pass
                else:
                    pass
                tmpa = pkg_index[0]
                ...
                lost_pkg_fg = False
        return rawdata_str
#        return copy.deepcopy(rawdata_str)
    ...
    def get_rawdata_packets_bromberg(self, path, step, fe_cfg_r, fembs_np = [0,1,2,3], val=100, jumbo_flag = True):
        numVal = int(val)
        if (numVal < 0) :
            print "FEMB_UDP--> Error record_hs_data: Invalid number of data packets requested"
            return None
